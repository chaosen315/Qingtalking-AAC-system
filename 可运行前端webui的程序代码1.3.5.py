...
def imgcaption(image_file,question):#这个函数输入图片输出文本
...
def eventdia(image, history, Dict):
        # 在这里编写处理图片和历史数据的代码
        #准备问题
        question1 = "What kind of place is the person in the photo at?"
        question2 = "What is the occupation of the person in the photo?"
        scene = imgcaption(image,question1)  # 这里通过imgcaption函数获得图像对应的场景“scene”
        identity = imgcaption(image,question2)  # 这里通过imgcaption函数获得图像对应的场景“identity”
        if history == []:#说明是新对话。

            # content = "假如我身处" + str(scene) + "，我有可能说些什么？请列举五句我可能说的话。"
            content = "假如我身处" + str(scene) + "并且想要和" + str(identity) + "开启对话。请使用中文列举五句我可能使用的开场白。输出要求:仅输出开场白。输出格式例子:1. 你好，请问你是美国人吗？"
            print(content)
        ...
        if history != []:#说明对话已经开始
            ...
            print(prompt_text)
            content = "我在"+str(scene)+"发生了对话，对方回复:" + str(callback_content) + "，我的态度是" + str(prompt_text) + "请给我5句补足态度背后论据的回复。输出格式例子:1. 谢谢你的建议，但是我想看看菜单上还有什么菜品可以选择。"
            print(content)
            dict_content = chatgpt_options(content)#这里将上一行组合的问题输送给chatgpt生成回答
            print(dict_content)
            Dict_content = dict_content
...
def update_chat_history(history):
    return history
dialogue_counter = 0
def endsave_dialog(history,Scene,Content,Dict):
    global dialogue_counter
    dialogue_counter += 1

    current_date = datetime.datetime.now().strftime("%Y-%m-%d")
    file_name = f"{current_date}_第{dialogue_counter}次对话.txt"
    ...
    Content = ""
    Dict = {}
    return history,Scene,Content,Dict,history
...
with gr.Blocks() as eventdia_demo:  # gradio的块布局。docs:https://gradio.app/docs/#blocks
    ...
    with gr.Row():  # 这是行布局,docs:https://gradio.app/docs/#block-layouts
        ...
        with gr.Column(scale=6):  # 这是列布局,docs:https://gradio.app/docs/#block-layouts
            # 在这里添加需要的组件，例如输入框、按钮等
            chatbot = gr.Chatbot(label="聊天记录展示",elem_id="chatbot").style(height=360)
            dictionary_output = gr.Textbox( lines=5, label="回复选项展示",
                                           elem_id='dict_display')
            optionbox = gr.Radio(choices=options, label="选择满意的对话选项",elem_id="option_box")  # docs:https://gradio.app/docs/#radio
            Contentdisplay = gr.Textbox(lines=4, label="提示文本展示", interactive=False,elem_id="Condisply")
            audiomsgdisplay = gr.Textbox(lines=4, label="录音对象回复文本展示", interactive=False, elem_id="audiomsgdis")
            send_message = gr.Button("发送", elem_id="send_message_button")  # docs:https://gradio.app/docs/#button
            end_this_dialog = gr.Button("结束对话", elem_id="end_this_dialog_button")

        with gr.Column(scale=3):  # 这是列布局
            # 在这里添加需要的组件，例如输入框、按钮等
            imagepath = gr.Image(source="upload", type="filepath", interactive=True, elem_id="image_path").style(
                width="275px", height="275px")  # docs:https://gradio.app/docs/#image
            dialogbutton = gr.Button("生成对话选项", elem_id="dialog_button")
            with gr.Row():
              ...
              with gr.Column(scale=1):
                attitude_int = gr.Button("确定态度", elem_id='attitude_value')
            ...
            audiomsgbutton = gr.Button("记录对象回复：语音转文本", elem_id="audio_msg_button")
    ...
    end_this_dialog.click(fn=endsave_dialog,inputs=[history,Scene,Content,Dict],outputs=[history,Scene,Content,Dict,chatbot])
